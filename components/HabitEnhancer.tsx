...
import { Loader2, Coffee, Sun, Moon, Briefcase, Heart, Activity, Dumbbell, Calendar, Apple, ChevronRight, Trash2, Plus, CheckCircle2, Circle, Clock, Zap, Utensils, Droplets, Scale, Lock, Flame, Check, Sparkles, Sprout, Drumstick } from 'lucide-react';
...
export const HabitEnhancer: React.FC<HabitEnhancerProps> = ({ onCompleteRoutine, onCompletePlan, onCompleteDiet, existingRoutine, existingPlan, existingDiet, targetCareer }) => {
  ...
  // Prepare chart data for routine
  ...
  // BMI scale: 15 to 35 covers most ranges of interest.
  const bmiPercent = Math.min(Math.max((bmiValue - 15) / (35 - 15) * 100, 0), 100);
  
  // Calculate needle angle for SVG (180 to 360 degrees, corresponding to 0 to 100%)
  // SVG coordinates: Center (50, 45). Arc from 180 deg to 0 deg (top half).
  // React gauge logic usually maps 0-100 to 180-0 degrees.
  const needleAngle = 180 - (bmiPercent / 100) * 180;
  const needleRad = (needleAngle * Math.PI) / 180;
  const needleLen = 35;
  const needleX = 50 + needleLen * Math.cos(needleRad);
  const needleY = 45 - needleLen * Math.sin(needleRad); // Subtract Y because SVG Y is down
      ...
      <div className={`flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 bg-gradient-to-r ${theme.gradient} p-6 rounded-3xl text-white shadow-lg ${theme.shadow} transition-all duration-500`}>
        ...
        <div className="bg-white/20 backdrop-blur-md p-1 rounded-xl flex self-start md:self-auto border border-white/20">
            <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center ${activeTab === 'routine' ? 'bg-white text-indigo-600 shadow-md' : 'text-white hover:bg-white/10'}`}
            >
            ...
            <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center ${activeTab === 'exercise' ? 'bg-white text-blue-600 shadow-md' : 'text-white hover:bg-white/10'}`}
            >
                <Dumbbell className="w-4 h-4 mr-2" />
                Fitness
            </button>
            <button 
                onClick={() => canAccessDiet && setActiveTab('diet')}
                disabled={!canAccessDiet}
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center ${activeTab === 'diet' ? 'bg-white text-teal-600 shadow-md' : 'text-white hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed'}`}
                title={!canAccessDiet ? "Complete Routine and Fitness first" : ""}
            >
                {canAccessDiet ? <Utensils className="w-4 h-4 mr-2" /> : <Lock className="w-4 h-4 mr-2" />}
                Diet
            </button>
        </div>
      </div>
          ...
          <div className="animate-in fade-in slide-in-from-left-4 duration-300">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 mb-8 flex flex-col md:flex-row gap-4 items-end">
                <div className="flex-1 w-full">
                    ...
                    <input
                        type="text"
                        className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-medium"
                        placeholder="e.g. Software Engineer, Writer, Marathon Runner"
                        value={careerInput}
                ...
                <button
                    ...
                    className={`w-full md:w-auto px-8 py-3 ${theme.button} text-white rounded-xl transition disabled:opacity-50 flex items-center justify-center whitespace-nowrap shadow-lg ${theme.shadow} font-bold`}
                >
                    ...
                    <div className="lg:col-span-7 space-y-6">
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-center mb-6 relative z-10">
                                ...
                                <span className={`text-xs ${theme.bgLight} ${theme.accent} px-3 py-1 rounded-full font-bold`}>
                            ...
                            <div className="relative z-10 space-y-4">
                                        ...
                                        <div key={index} className="relative pl-10 group">
                                            ...
                                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/30 transition border border-transparent hover:border-gray-200 dark:hover:border-gray-700">
                                                ...
                                                <input 
                                                    ...
                                                    className="w-24 bg-transparent font-mono text-xs font-bold text-gray-500 dark:text-gray-400 focus:text-indigo-600 dark:focus:text-indigo-400 focus:outline-none text-right sm:text-left"
                                                />
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <input 
                                                        ...
                                                        className="w-full bg-transparent text-gray-900 dark:text-white font-medium focus:outline-none text-sm border-b border-transparent focus:border-indigo-300 dark:focus:border-indigo-700 transition-colors px-1"
                                                    />
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2 mt-2 sm:mt-0">
                                                    <select 
                                                        ...
                                                        className={`text-xs font-bold px-2 py-1 rounded-lg cursor-pointer outline-none border transition-colors ${styles.bg} ${styles.text} ${styles.border}`}
                                                    >
                                ...
                                <button 
                                    ...
                                    className="relative ml-10 w-[calc(100%-2.5rem)] py-3 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl text-sm font-bold text-gray-400 hover:text-indigo-500 hover:border-indigo-300 dark:hover:border-indigo-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/10 flex items-center justify-center transition"
                                >
                                        ...
                                        <PieChart>
                                            ...
                                            >
                                                {chartData.map((entry, index) => (
                                                    <Cell key={`cell-${index}`} fill={getCategoryStyles(entry.name).hex} />
                                                ))}
                                            </Pie>
                                            <RechartsTooltip 
                                                contentStyle={{ backgroundColor: '#1f2937', borderColor: '#374151', color: '#fff', borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}
                                ...
                                {routine.habits.map((habit, index) => {
                                    ...
                                    return (
                                        <div 
                                            ...
                                            className={`p-3 rounded-xl backdrop-blur-sm border transition-all cursor-pointer flex items-start gap-3 group ${
                                                isDone 
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className={`text-sm font-bold transition-opacity ${isDone ? 'opacity-50 line-through' : 'opacity-100'}`}>
                                                    {habit.name}
                                                </div>
                                                <div className="flex justify-between items-center mt-1">
                                                    <span className="text-xs text-indigo-200/80">{habit.duration}</span>
                                                    <span className="text-[10px] uppercase tracking-wider font-bold bg-black/20 px-1.5 py-0.5 rounded text-white/70">
                                                        {habit.type}
                                                    </span>
                                                </div>
                                            </div>
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 mb-8 flex flex-col md:flex-row gap-4 items-end">
                <div className="flex-1 w-full">
                    ...
                    <input
                        type="text"
                        className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-medium"
                        placeholder="e.g. Build Muscle, Lose Weight, Yoga for Flexibility"
                        value={fitnessGoal}
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-300">
               ...
               <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 mb-8 flex flex-col md:flex-row gap-4 items-end">
                <div className="w-full md:w-1/3">
                    ...
                    <input
                        type="number"
                        className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl focus:ring-2 focus:ring-teal-500 outline-none font-medium"
                        placeholder="e.g. 175"
                        value={height}
                ...
                <button
                    ...
                    disabled={dietLoading || !height || !weight}
                    className={`w-full md:w-auto px-8 py-3 ${theme.button} text-white rounded-xl transition disabled:opacity-50 flex items-center justify-center whitespace-nowrap shadow-lg ${theme.shadow} font-bold`}
                >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                     ...
                     <div className="lg:col-span-4 space-y-6">
                        ...
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 text-center relative overflow-hidden">
                             ...
                             <div className="relative h-24 mx-auto w-48 mb-2">
                                <svg viewBox="0 0 100 60" className="w-full h-full">
                                    ...
                                    <path 
                                        ...
                                        strokeWidth="8" 
                                        strokeLinecap="round" 
                                        strokeDasharray="126" // Approx length of the arc
                                        strokeDashoffset={126 - (126 * bmiPercent) / 100}
                             ...
                             <div className="inline-block px-4 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-bold text-sm border border-gray-200 dark:border-gray-600">
                                {dietPlan.bmiCategory}
                            </div>
                        </div>

                        {/* Improved Daily Targets Box */}
                        <div className="bg-emerald-50 dark:bg-emerald-900/10 p-6 rounded-3xl border border-emerald-100 dark:border-emerald-800/30 overflow-hidden">
                            <h3 className="flex items-center text-lg font-bold text-emerald-800 dark:text-emerald-300 mb-6">
                                <Flame className="w-5 h-5 mr-2 text-emerald-500" /> Daily Targets
                            </h3>
                            
                            {/* Fixed Calories Layout */}
                        ...
                        <div className="bg-blue-50 dark:bg-blue-900/10 p-6 rounded-3xl border border-blue-100 dark:border-blue-900/30">
                             <h3 className="flex items-center text-lg font-bold text-blue-800 dark:text-blue-300 mb-2">
                                <Droplets className="w-5 h-5 mr-2" /> Hydration
                            </h3>
                     ...
                     <div className="lg:col-span-8 space-y-6">
                         ...
                         {/* Daily Tracker & Preference Banner */}
                             ...
                             <div className="flex-1 bg-gradient-to-r from-teal-500 to-emerald-500 rounded-2xl p-6 text-white shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4">
                                 <div>
                                     <h3 className="text-lg font-bold flex items-center">
                                         <Sparkles className="w-5 h-5 mr-2" /> Today's Progress
                                     </h3>
                                     <p className="text-teal-100 text-sm opacity-90">Track your daily adherence.</p>
                                 </div>
                                 <button 
                                    onClick={toggleDietComplete}
                                    className={`flex items-center px-4 py-2 rounded-xl font-bold transition-all shadow-sm text-sm whitespace-nowrap ${
                                        dailyDietComplete 
                                        ? 'bg-white text-teal-600' 
                                        : 'bg-black/20 hover:bg-black/30 text-white'
                                    }`}
                                 >
                                     <div className={`w-4 h-4 rounded-full border-2 mr-2 flex items-center justify-center ${dailyDietComplete ? 'border-teal-600' : 'border-white'}`}>
                                         {dailyDietComplete && <Check className="w-2.5 h-2.5" />}
                                     </div>
                         ...
                         <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                                <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center">
                                    <Utensils className="w-6 h-6 mr-2 text-teal-500" /> Daily Meal Plan
                                </h3>
                                ...
                                <div className="flex bg-gray-100 dark:bg-gray-700/50 p-1 rounded-xl self-stretch sm:self-auto">
                                    <button
                                        ...
                                        className={`flex-1 sm:flex-none flex items-center justify-center px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                                    ...
                                    <button
                                        onClick={() => setDietPreference('mixed')}
                                        className={`flex-1 sm:flex-none flex items-center justify-center px-4 py-2 rounded-lg text-sm font-bold transition-all ${
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300" key={dietPreference}>
                                 <div className="p-5 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-teal-200 dark:hover:border-teal-900 transition-colors">
                                     <div className="flex justify-between items-center mb-3 border-b border-gray-200 dark:border-gray-700 pb-2">
                                         <span className="text-xs font-bold text-gray-400 uppercase tracking-wide">Breakfast</span>
                                         <span className="text-xs font-bold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 px-2 py-1 rounded">8:00 AM</span>
                                     </div>
                                     {Array.isArray(currentMeals.breakfast) 
                                        ? renderMealList(currentMeals.breakfast) 
                                        : <p className="text-sm text-gray-500 italic">No meals found.</p>
                                     }
                                 </div>

                                 <div className="p-5 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-teal-200 dark:hover:border-teal-900 transition-colors">
                                     <div className="flex justify-between items-center mb-3 border-b border-gray-200 dark:border-gray-700 pb-2">
                                         <span className="text-xs font-bold text-gray-400 uppercase tracking-wide">Lunch</span>
                                         <span className="text-xs font-bold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 px-2 py-1 rounded">1:00 PM</span>
                                     </div>
                                     {Array.isArray(currentMeals.lunch) 
                                        ? renderMealList(currentMeals.lunch) 
                                        : <p className="text-sm text-gray-500 italic">No meals found.</p>
                                     }
                                 </div>
                                 ...
                                 <div className="p-5 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-teal-200 dark:hover:border-teal-900 transition-colors">
                                     <div className="flex justify-between items-center mb-3 border-b border-gray-200 dark:border-gray-700 pb-2">
                                         <span className="text-xs font-bold text-gray-400 uppercase tracking-wide">Snack</span>
                                         <span className="text-xs font-bold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 px-2 py-1 rounded">4:00 PM</span>
                                     </div>
                                     {Array.isArray(currentMeals.snack) 
                                        ? renderMealList(currentMeals.snack) 
                                        : <p className="text-sm text-gray-500 italic">No meals found.</p>
                                     }
                                 </div>
                                 ...
                                 <div className="p-5 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-teal-200 dark:hover:border-teal-900 transition-colors">
                                     <div className="flex justify-between items-center mb-3 border-b border-gray-200 dark:border-gray-700 pb-2">
                                         <span className="text-xs font-bold text-gray-400 uppercase tracking-wide">Dinner</span>
                                         <span className="text-xs font-bold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 px-2 py-1 rounded">8:00 PM</span>
                                     </div>
                                     {Array.isArray(currentMeals.dinner) 
                                        ? renderMealList(currentMeals.dinner) 
                                        : <p className="text-sm text-gray-500 italic">No meals found.</p>
                                     }
                                 </div>
                             </div>
